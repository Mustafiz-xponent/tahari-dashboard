...
import { useDeletePromotionMutation } from "@/redux/services/promotionApi";

const PromotionDeleteDialog = ({ promotionId }: { promotionId: string }) => {
  const [deletePromotionHandler, { isLoading }] = useDeletePromotionMutation();

  const submitHandler = () => async () => {
    try {
      const response = await deletePromotionHandler(promotionId).unwrap();
      if (response?.success) {
