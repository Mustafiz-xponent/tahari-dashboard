...
// import { useCreateOrderTrackingMutation } from "@/redux/services/orderTrackingsApi";
// import { useGetAllOrdersQuery } from "@/redux/services/ordersApi";
// import { createOrderTrackingSchema } from "@/lib/validations/orderTrackingSchema";
...
// const AddOrderTrackingForm = () => {
//   const [addOrderTrackingHandler, { isLoading }] = useCreateOrderTrackingMutation();
//   const { data: ordersData, isLoading: ordersLoading } = useGetAllOrdersQuery({
//     limit: 100,
//   });

//   const form = useForm<z.infer<typeof createOrderTrackingSchema>>({
//     resolver: zodResolver(createOrderTrackingSchema),
//     defaultValues: {
//       orderId: 0,
...
//   async function onSubmit(data: z.infer<typeof createOrderTrackingSchema>) {
//     const submitData = {
//       ...data,
...
//     };
//     try {
//       const response = await addOrderTrackingHandler(submitData).unwrap();
//       if (response.success) {
//         toast.success(response?.message || "Order tracking added successfully.");
//         form.reset();
//       }
//     } catch (err: unknown) {
...
// export default AddOrderTrackingForm;
...
const AddOrderTrackingForm = () => {
  ...
  const [addOrderTrackingHandler, { isLoading }] =
  ...
  async function onSubmit(data: z.infer<typeof createOrderTrackingSchema>) {
    try {
      const response = await addOrderTrackingHandler(data).unwrap();
      if (response.success) {
        toast.success(
          response?.message || "Order tracking added successfully."
        );
        form.reset();
